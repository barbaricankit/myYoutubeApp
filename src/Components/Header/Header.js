import { faBars, faChevronLeft, faPlay, faSearch, faSignInAlt } from '@fortawesome/free-solid-svg-icons';
...
const Header = () => {
	...
	const { authState: { login, userInitials }, authDispatch } = useAuth();
		...
		<header>
			...
			<div className='page_header'>
				<div className='menu_elem'>
					...
					<NavLink end className='menu__item nav__item' activeStyle={{ color: 'lightblue' }} to='/'>
				...
				<div className='user'>
					...
					{login && (
						<span ref={userRef} onClick={() => setShowOptions((prevValue) => !prevValue)}>
							<span className='single-letter-avatar avatar-sm'>{userInitials}</span>
						</span>
					)}
					{!login && (
