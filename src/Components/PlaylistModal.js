...
const PlaylistModal = () => {
  ...
  const isPlaylistCheckboxChecked = (playlist) =>
    state.playVideo.playlists.find((playlst) => playlst === playlist) ===
    undefined
      ? false
      : true;
  ...
  const managePlaylistAndVideoList = (playlist) => {
    dispatch({
      ...
      value: {
        videos: addVideoToPlayList(state.playVideo, playlist),
        playlistname: playlist,
      },
    });
    dispatch({
      type: "SELECTED_PLAYLIST",
      value: { playlistName: playlist, video: state.playVideo },
    });
  };
  const PlaylistNameList = () => {
    return (
      <>
        <li key='0' className='stacked-list-item'>
          Save to..{" "}
          <span
            className='badges'
            onClick={() => {
        ...
        {PLAYLIST_NAMES.map((playlist, index) => {
          return (
            <li key={index + 1} className='stacked-list-item'>
              <input
                className='btn-toast-1'
                type='checkbox'
    ...
    if (!showNewPlayListInput)
      return (
        <li
          ...
          <button className='btn-text'>+</button> Create New Playlist
        </li>
      );
    return (
      <li className='stacked-list-item'>
        ...
        <button
          className='btn-text'
          disabled={newPlaylistText === "" ? true : false}
          onClick={() => {
            dispatch({
              type: "NEW_PLAYLIST_ADDED",
              value: { playlistName: newPlaylistText, video: state.playVideo },
            });
            dispatch({
              type: "ADDED_VIDEO_TO_PLAYLIST",
              value: {
                videos: addVideoToPlayList(state.playVideo, newPlaylistText),
                playlistname: newPlaylistText,
              },
            });
            dispatch({ type: "PLAYLIST_OPTIONS" });
  ...
  return (
    <>
      <div
        ...
        {addtoPlaylist ? (
          <div className={showToast}>Added to {playlistSelected}</div>
        ) : (
          <div className={showToast}>Removed from {playlistSelected}</div>
        )}
      </div>
    </>
  );
