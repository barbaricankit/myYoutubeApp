    ...
    <button className="close-modal" onClick={() => setShowModal(false)}>x</button>
            ...
            <div className="btn-modal">
                ...
                <button className="btn-primary" onClick={() => {
                    ...
                    dispatch({ type: "REMOVE_VIDEO_FROM_WATCH_LATER", value: video });} }>Remove</button>
            </div>
        </div>
    return <>
    <div>
        ...
        
        {
            ...
            state.watchlater.map((video,index)=><div key={index} className="card horizontal-card-with-text" style={{ margin: "1rem" }} >
        ...
        {state.watchlater.length===0 && <div className="h4" style={{margin:"4rem"}}>Add Videos to watch later</div>}
    </div>
    </>
}