...
export const managePlaylistReducer = (state, action) => {
	switch (action.type) {
		...
		case 'UPDATE_INITIAL_STATE':
			const { likedVideos, dislikedVideos, playLists, watchlater, historyVideos } = action.value;
			return {
				...state,
				playLists,
		...
		case 'ADD_VIDEO_TO_HISTORY':
			return {
				...state,
				history: [ action.value, ...state.history.filter((id) => id !== action.value) ]
			};
		...
		case 'SELECTED_PLAYLIST':
			...
			const current_playlist = state.playLists.find(({ playlistName }) => playlistName === playlist_name);
		...
		case 'LIKED_VIDEO':
			return {
				...
				dislikedVideos: findDisLikeVideo(state, action.video_id)
		...
		case 'DISLIKED_VIDEO':
			return {
				...
				dislikedVideos: findDisLikeVideo(state, action.video_id)
					? filterDisLikedVideo(state, action.video_id)
					: [ ...state.dislikedVideos, action.video_id ],
				likedVideos: findLikedVideo(state, action.video_id)
					? filterLikedVideo(state, action.video_id)
					: state.likedVideos
			};
		...
		case 'DELETE_PLAYLIST':
			return {
				...state,
				playLists: state.playLists.filter(({ playlistName }) => playlistName !== action.playlistName)
			};
